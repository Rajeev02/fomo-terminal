  ...
  return (
    <div className="relative isolate flex flex-col min-h-screen bg-bg-primary overflow-x-hidden selection:bg-[var(--chad-green)] selection:text-black">
      ...
      <main className="flex flex-col items-center justify-center flex-1 h-full w-full">
        ...
        <div className="flex flex-col items-center gap-5 md:gap-8 mt-20 md:mt-32">
          <div className="flex flex-col gap-2 items-center text-center px-6">
            ...
            <p className="md:text-[22px] text-zinc-400 text-center md:leading-8 tracking-tight max-w-2xl mt-4">
              From memecoins to yield farming, the most premium wallet on Solana. Built for the culture.
            </p>
          </div>
          
          <div className="flex gap-2 md:hidden mt-6">
            <Link href="#" className="text-center z-10 bg-white/10 backdrop-blur-md border border-white/10 rounded-xl text-lg font-bold w-48 py-3 text-white hover:bg-white/20 transition-colors">
              Download app
            </Link>
          </div>
          
          <div className="hidden md:flex gap-4 mt-8">
            <button className="group relative items-center justify-center overflow-hidden bg-[var(--chad-green)]/20 hover:bg-[var(--chad-green)]/40 text-[var(--chad-green)] backdrop-blur-md transition-colors duration-150 py-4 px-8 w-56 rounded-xl text-lg font-bold border border-[var(--chad-green)]/30 z-10 flex">
            ...
            <button className="z-10 group bg-white/5 hover:bg-white/10 backdrop-blur-md transition-colors duration-150 border border-white/10 text-white rounded-xl text-lg font-bold w-56 flex items-center justify-center overflow-hidden">
              <div className="flex items-center overflow-hidden w-0 opacity-0 group-hover:w-7 group-hover:opacity-100 transition-all duration-150 ease-out">
        ...
        <div className="hidden md:block w-24 h-24 rounded-full bg-gradient-to-br from-purple-500 to-[var(--chad-green)] blur-2xl opacity-20 absolute top-60 right-32 animate-float-desktop" style={{ animationDelay: '1s' }} />

        {/* Cross Platform Section */}
        <div className="hidden md:flex flex-col items-center pt-32 pb-24 px-8 gap-3 w-full">
          <div className="font-mono font-bold text-[var(--chad-purple)] tracking-widest text-sm">NOW ON DESKTOP & MOBILE</div>
          ...
          <div className="relative mt-16 w-full max-w-5xl flex justify-center h-[400px]">
            <div className="w-[800px] h-[450px] bg-bg-secondary rounded-t-2xl border-t border-l border-r border-white/10 shadow-2xl overflow-hidden relative">
              ...
              <div className="mt-8 p-8 h-full bg-gradient-to-b from-bg-tertiary to-bg-primary opacity-50 flex items-center justify-center">
                <span className="font-mono text-zinc-600">[DESKTOP UI MOCKUP]</span>
              </div>
            </div>
            <div className="w-[260px] h-[520px] bg-black rounded-[3rem] border-[8px] border-zinc-800 shadow-2xl absolute -right-4 bottom-[-100px] animate-float-desktop z-20 overflow-hidden">
               <div className="w-full h-full bg-gradient-to-b from-bg-tertiary to-bg-primary opacity-80 flex items-center justify-center">
                 <span className="font-mono text-zinc-600 text-sm">[MOBILE UI MOCKUP]</span>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="pt-24 md:py-32 px-4 md:px-20 flex flex-col self-stretch md:self-center gap-12 w-full max-w-[1400px] z-10">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col md:flex-row gap-6 items-stretch">
              <FeatureCard label="PORTFOLIO" title="track your wealth in real-time" placeholderColor="bg-blue-500/10" />
              <FeatureCard label="DEFI YIELD" title="stake and earn while you sleep" placeholderColor="bg-[var(--chad-green)]/10" />
              <FeatureCard label="MEMECOIN SNIPER" title="never miss a 100x gem again" placeholderColor="bg-[var(--chad-purple)]/10" />
            </div>
            <div className="flex flex-col md:flex-row gap-6 items-stretch">
              <FeatureCard label="EASY ONBOARDING" title="create a wallet in an instant" placeholderColor="bg-yellow-500/10" />
              <FeatureCard label="ZERO GAS" title="sponsored transactions built in" placeholderColor="bg-red-500/10" />
              <FeatureCard label="ONE CLICK" title="fund easily with apple pay" placeholderColor="bg-cyan-500/10" />
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-col justify-center items-center relative">
              <div className="flex flex-col gap-6 items-center w-full relative z-30">
                <h2 className="text-[40px] leading-[44px] md:text-[72px] md:leading-[76px] tracking-tighter text-center font-black">
                  a crypto wallet<br/>for the rest of us
                </h2>
                ...
                <div className="pt-10 w-full flex justify-center">
                  <div className="flex gap-2 md:hidden">
                    <Link href="#" className="text-center z-10 bg-[var(--chad-green)] text-black rounded-xl text-lg font-bold w-50 py-3">
                      Download app
                    </Link>
                  </div>
                  <div className="hidden md:flex gap-4">
                    <button className="group relative items-center justify-center overflow-hidden bg-[var(--chad-green)] hover:bg-[var(--chad-green)]/90 text-black transition-colors duration-150 py-4 px-8 w-56 rounded-xl text-lg font-bold z-10 flex">
                      <span>Launch Web App</span>
                      <div className="flex items-center overflow-hidden w-0 opacity-0 group-hover:w-7 group-hover:opacity-100 transition-all duration-150 ease-out">
                        <ArrowRight className="size-5 ml-2 shrink-0" />
                      </div>
